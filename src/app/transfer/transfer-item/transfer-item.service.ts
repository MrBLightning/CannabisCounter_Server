...
export class TransferItemService {
    ...
    async addTransfer(netw: string, record: Order): Promise<number> {
        const conn = await this.mysql.getConnection(netw);
        // this query returns the id of the new line created
        const results: number = await conn.query(`INSERT INTO ${APP_TABLES.TRANSFER} SET ?`, [record]);

        if (!results)
            throw new Error("Transfer not added.");
        return results;
    };
    ...
    async deleteOrder(netw: string, Id: number): Promise<number> {
        ...
        if (!results)
            throw new Error("Transfer not deleted.");
        return results;
    };

...
const ACTION = 'order/reserveChicken';
@Controller(ACTION)
...
export class OrderReserveChickenController {
  @Inject(OrderReserveChickenService) private readonly orderReserveChickenService: OrderReserveChickenService;
...
//   @Inject(AppLogger) private readonly appLogger: AppLogger;
  ...
  async getSiryuns(@ReqUser() user: AuthUser) {
    return await this.orderReserveChickenService.getSiryuns(user.netw);
  }
  ...
  async getSiryunTable(@ReqUser() user: AuthUser, @Param('searchDate') searchDate: number) {
    ...
    // create filter value: ClassId == 1 (Department עוף)
    let ClassId = 1;
    ...
    let siryuns = await this.orderReserveChickenService.getSiryunByDate(user.netw, moment(SearchDate).format('YYYY-MM-DD'));
    ...
    for (let i = 0; i < lengthCatalogs; i++) {
      ...
      if (typeof siryunBar[0] === 'undefined') {
        for (let n = 0; n < supSiryuns.length; n++) {
          ...
          await this.orderReserveChickenService.addSiryun(user.netw, record);
        }
      } else {
  ...
  async getSiryunByDate(@ReqUser() user: AuthUser, @Param('CreateDate') CreateDate: string) {
    return await this.orderReserveChickenService.getSiryunByDate(user.netw, CreateDate);
  }
  ...
  async addSiryun(@ReqUser() user: AuthUser, @Body('record') record: Siryun) {
    ...
    // });
    return await this.orderReserveChickenService.addSiryun(user.netw, record);
  }
  ...
  async updateSiryun(@ReqUser() user: AuthUser, @Body('record') record: SiryunUpdate, @Body('recordWhere') recordWhere: SiryunWhere) {
    ...
    //   branchId: user.branches[0]
    // });
    return await this.orderReserveChickenService.updateSiryun(user.netw, recordWhere, record);
  }
  ...
  async deleteSiryun(@ReqUser() user: AuthUser, @Param('Id') Id: number) {
    ...
    //   branchId: user.branches[0]
    // });
    return await this.orderReserveChickenService.deleteSiryun(user.netw, Id);
  }
}

...
@Injectable()
export class ManageConversionService {
    ...
    async addConversion(netw: string, record: Conversion): Promise<number> {
        const conn = await this.mysql.getConnection(netw);
        // this query returns the id of the new line created
        const results: number = await conn.query(`INSERT INTO ${APP_TABLES.CONVERSION} SET ?`, [record]);
        
        if (!results)
            throw new Error("Conversion not added.");
        return results;
    };

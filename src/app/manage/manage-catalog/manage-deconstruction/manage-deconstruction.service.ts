...
@Injectable()
export class ManageDeconstructionService {
    ...
    async addDeconstruction(netw: string, record: Deconstruction): Promise<number> {
        const conn = await this.mysql.getConnection(netw);
        // this query returns the id of the new line created
        const results: number = await conn.query(`INSERT INTO ${APP_TABLES.DECONSTRUCTION} SET ?`, [record]);
        
        if (!results)
            throw new Error("Deconstruction not added.");
        return results;
    };

...
import { Siryun, SiryunUpdate, SiryunWhere } from 'src/shared/types/system.types';
...
@Injectable()
export class ManageSiryunService {
    ...
    // async getSiryunById(netw: string, Id: number, record: Siryun): Promise<Siryun[]> {
    //     const conn = await this.mysql.getConnection(netw);
    //     let results: Siryun[];
    //     if (Id > 0) {
    //         // this query returns a single line object
    //         results = await conn.query(`SELECT * FROM ${APP_TABLES.SIRYUN} WHERE Id = ${escape(Id + "")} LIMIT 1`);
    //     } else {
    //         // this query returns multiple lines 
    //         results = await conn.query(`SELECT * FROM ${APP_TABLES.SIRYUN} WHERE ?`, [record]);
    //     }
    ...
    async addSiryun(netw: string, record: Siryun): Promise<number> {
        const conn = await this.mysql.getConnection(netw);
